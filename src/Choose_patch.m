...
%% Add Total Matter Delta at z=1000
...
datstat = [stdDc stdTc rmsVc*MpcMyr_2_kms stdDb stdTb rmsVb*MpcMyr_2_kms rmsVcb*MpcMyr_2_kms Vcbp*MpcMyr_2_kms stdT stdDm];
...
%% Add Total Matter Delta at zend
...
datstat = [sDc_azend sTc_azend sDb_azend sTb_azend rmsVcb_azend*MpcMyr_2_kms Vcbp_azend*MpcMyr_2_kms sT_azend sDm_azend];
...
elseif choose_zi_flag
  ...
  relerr = (Delta_m(indices_patch)-odnum).^2/stdDm^2 + (Vcb(indices_patch)-Vcbnum).^2/rmsVcb^2;
  [minrelerr, indmin] = min(relerr);
  ind_patch = indices_patch(indmin);
  disp(['Wanted Delta_m = ' num2str(odnum) '; Selected patch''s Delta_m = ' num2str(Delta_m(ind_patch))]);
  ...
  %% Find the best matching patch
  disp('----------------One best matching patch is being found----------------');
  relerr = (Dm3D_azend(indices_patch)-odnum).^2/sDm_azend^2 + (Vcb_azend(indices_patch)-Vcbnum).^2/rmsVcb_azend^2;
  [minrelerr, indmin] = min(relerr);
  ind_patch = indices_patch(indmin);
  disp(['Wanted Delta_m = ' num2str(odnum) '; Selected patch''s Delta_m = ' num2str(Dm3D_azend(ind_patch))]);
